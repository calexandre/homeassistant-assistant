...
2. **Fetch Home Assistant Configs**
   - Invoke the `ha-config-fetch` skill to refresh and read the latest automations, scenes, scripts, configuration YAML, and logs from the live instance
...
7. **Backward-Incompatible Changes Assessment**
   ...
   Cross-reference each breaking change against **both** the live entity context (from `GetLiveContext`) **and** the fetched config files (via the `ha-config-fetch` skill):
   - **Integration**: Name
